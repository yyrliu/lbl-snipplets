...
def convert_file(opt):
    ...
    for i in range(nb):
        ...
        for j in range(nrow):
            values = [
                "%.6f" % block.get_column(k).get_value(j) for k in range(1, ncol + 1)
            ]
            f.write("\t".join(values) + "\n")
...
def main():
    ...
    parser.add_argument("-l", action="store_true", help="list supported file types")
    parser.add_argument(
        "-i", metavar="FILETYPE", help="show information about filetype"
    )
    parser.add_argument("-s", action="store_true", help="do not output metadata")
    parser.add_argument("-t", metavar="TYPE", help="specify filetype of input file")
