...
def load_gamma_data(gamma_files, label_mapping):
    ...
    for gamma_angle, files in gamma_groups.items():
        ...
        for idx, gamma_file in enumerate(files_sorted, 1):
            ...
            try:
                df = pd.read_csv(
                    gamma_file, sep="\t", comment="#", names=["gamma", "intensity"]
                )
            except Exception:
                # Fallback for different file formats
                df = pd.read_csv(
                    gamma_file, sep="\t", header=None, names=["gamma", "intensity"]
                )
            ...
            df = (
                df[(df.index > -150) & (df.index < 150)]
                if "gamma" in str(gamma_file)
                else df[(df.index > 4) & (df.index < 50)]
            )
...
def perform_peak_fitting(df, peak_center_guess, cols):
    ...
    for col in cols:
        ...
        if result["success"]:
            ...
            print(
                f"{col:30s} | FWHM: {result['fwhm']:.3f}° | Center: {result['center']:.2f}° | R²: {result['r_squared']:.4f}"
            )
        else:
...
def plot_gamma_analysis(df, plot_cfg, data_dir=None):
    ...
    if fwhm_results:
        ...
        print(
            f"Peak center range: {min(center_values):.2f}° to {max(center_values):.2f}°"
        )
        print(
...
def main():
    ...
    # Find gamma files
    gamma_files = find_gamma_files(gamma_cfg["data_dirs"], gamma_cfg["file_pattern"])
