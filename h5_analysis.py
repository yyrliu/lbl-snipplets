...
def filter_h5_files(h5_files):
    ...
    for file_path in h5_files:
        ...
        if not match:
            ignored_files.append((filename, "Does not follow pattern XXX_run{n}_spec_run.h5"))
            continue
        ...
        if sample_id not in filtered_files or run_number > filtered_files[sample_id]['run_number']:
            # If we're replacing a file, add the old one to ignored list
            if sample_id in filtered_files:
                old_filename = Path(filtered_files[sample_id]['file_path']).name
                ignored_files.append((old_filename, f"Lower run number ({filtered_files[sample_id]['run_number']}) than {filename} (run {run_number})"))
            ...
            ignored_files.append((filename, f"Lower run number ({run_number}) than existing file with run {filtered_files[sample_id]['run_number']}"))

    return filtered_files, ignored_files


def discover_h5_files(data_dirs, verbose=True):
    ...
    if verbose:
        ...
        if ignored_files:
            ...
            for filename, reason in ignored_files:
...
def process_uv_vis_data(wl_data, wls_range=None):
    ...
    # Data processing
    time_nornalized_transmitted_intensity = wl_samp_spectrum/wl_samp_int_times.reshape((8,1)) - wl_dark_spectrum/wl_dark_int_time
    average_transmitted_intensity = np.mean(time_nornalized_transmitted_intensity, axis=0)  # Average individual spectra per sample
    time_nornalized_reference = wl_ref_spectrum/wl_ref_int_time - wl_dark_spectrum/wl_dark_int_time
    transmittance = average_transmitted_intensity / time_nornalized_reference  # Divide by the reference
...
def process_pl_data(pl_data, wls_range=None):
    ...
    for i in range(spectra.shape[0]):
        # Time normalization
        time_normalized_intensity = np.divide(spectra[i], spec_int_times[i]) - np.divide(dark_spectrum, dark_int_time)
    ...
    if processed_spectra:
        ...
        transformed_avg_signal = apply_jacobian(avg_spectrum, processed_spectra[0]['wavelength'])
...
def plot_uv_vis_spectra(uv_vis_data, sample_id, save_path=None):
    ...
    ax3.plot(uv_vis_data['energy'], uv_vis_data['absorption_jacobian'], 'g-', linewidth=2)
...
def plot_pl_spectra(pl_data, sample_id, save_path=None):
    ...
    for i, spectrum in enumerate(pl_data['processed_spectra']):
        ax1.plot(spectrum['wavelength'], spectrum['intensity'], 
                color=colors[i], alpha=0.7, label=f'Position {i+1}')
    ...
    for i, spectrum in enumerate(pl_data['processed_spectra']):
        ax2.plot(spectrum['energy'], spectrum['intensity_jacobian'], 
                color=colors[i], alpha=0.7, label=f'Position {i+1}')
...
def analyze_h5_file(file_path, plot_results=True, save_plots=False, uv_vis_range=None, pl_range=None):
    ...
    print(f"Mean Transmittance (550-800 nm): {uv_vis_results['mean_transmittance']:.4f}")
...
def analyze_multiple_files(file_list, max_files=5, uv_vis_range=None, pl_range=None):
    ...
    for i, file_path in enumerate(file_list[:max_files]):
        ...
        result = analyze_h5_file(file_path, plot_results=False, 
                               uv_vis_range=uv_vis_range, pl_range=pl_range)
        if result:
...
def create_summary_dataframe(results):
    ...
    for result in results:
        summary_data.append({
            ...
            'Integrated Transmittance': result['uv_vis_results']['integrated_transmittance'],
...
if __name__ == "__main__":
    ...
    if len(h5_files) > 0:
        ...
        print("Please update DEFAULT_DATA_DIRS or use discover_h5_files() with your data paths.")
